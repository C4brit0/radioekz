...
(() => {
  // ---- CONFIG ----
  const reacts = {
    "fail": "https://files.catbox.moe/868eet.mp3",
    "rapaz": "https://files.catbox.moe/b4s6v8.mp3",
    "kassino": "https://files.catbox.moe/43ulc0.mp3",
    "muehehe": "https://files.catbox.moe/3ihv3c.mp3",
    "chicote": "https://files.catbox.moe/0il6r5.mp3",
    "ahh": "https://files.catbox.moe/c1lzrf.mp3",
    "lesgo": "https://files.catbox.moe/115y9d.mp3",
    "brasil": "https://files.catbox.moe/h969qt.mp3",
    "seprepara": "https://files.catbox.moe/rnth7g.mp3",
    ...
    "macaco": "https://files.catbox.moe/zqxp33.mp3",
    "fiufiu": "https://files.catbox.moe/chtjex.mp3",
    "madruga": "https://files.catbox.moe/d0d0we.mp3",
    "beeh": "https://files.catbox.moe/vtyce4.mp3",
    "maru": "https://files.catbox.moe/0nwzcg.mp3",
    "mosquito": "https://files.catbox.moe/lq8qw3.mp3",
    "bejim": "https://files.catbox.moe/as4e3x.mp3",
    "fome": "https://files.catbox.moe/dulris.mp3",
    "fonfon": "https://files.catbox.moe/faswt9.mp3",
    "tadaima": "https://files.catbox.moe/mtb7ib.mp3",
    "okaeri": "https://files.catbox.moe/zjhl3e.mp3",
    "sabz": "https://files.catbox.moe/edx0uv.mp3",
  };

  const pingMentionUrl = 'https://files.catbox.moe/dydrge.mp3'; // som para menção
  let username = null;
  ...
  function playReactByName(name) {
    if (!enabled) return;
    if (!reacts[name]) {
      console.warn(`[reacts] react "${name}" não encontrado`);
      return;
    }
    const audio = audioCache[name].cloneNode();
    audio.volume = 0.5;
    audio.play().catch(() => {
      console.warn(`[reacts] falha ao tocar o react "${name}"`);
    });
  }

  // ---- COMANDOS ----
  function extractReactNames(text) {
    if (!text) return [];
    text = text.trim();
    const out = [];
    if (text.startsWith("/r")) {
      ...
      if (rest) {
        rest.split(/\s+/).forEach(n => {
          const lower = n.toLowerCase();
          if (reacts[lower]) {
            out.push(lower);
          } else {
            console.warn(`[reacts] comando react desconhecido: "${n}"`);
          }
        });
      }
      ...
      if (reacts[cmd]) {
        ...
        console.warn(`[reacts] comando curto react desconhecido: "${cmd}"`);
      }
    }
    return out;
  }
  ...
  // ---- BOTÃO DE ATIVAÇÃO ----
  function makeEnableButton() {
    if (document.getElementById("enable-reacts-btn")) return;
    const btn = document.createElement("button");
    btn.id = "enable-reacts-btn";
    btn.textContent = "Ativar Reações (clique 1x)";
    ...
    btn.onclick = () => {
      enabled = true;
      btn.textContent = "Reações Ativadas";
      setTimeout(() => btn.remove(), 1200);
      console.log("[reacts] Reações ativadas");
    };
    document.body.appendChild(btn);
  }

  // ---- OBSERVADOR DE MENSAGENS NO CHAT ----
  function initObserver() {
    const container = document.getElementById("messagebuffer");
    if (!container) {
    ...
    const mo = new MutationObserver(mutations => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          ...
          const messageSpan = node.querySelector('span:last-of-type');
          ...
          // Para debug
          console.log('[reacts] mensagem limpa:', messageText);
          console.log('[reacts] nick:', usernameText);

          const names = extractReactNames(messageText);
          if (names.length) {
            names.forEach(n => playReactByName(n));
          }

          // Som de menção para o usuário
          if (username && messageText.toLowerCase().includes(username.toLowerCase())) {
            playAudio(pingMentionUrl);
          }
        }
      }
    });
    mo.observe(container, { childList: true, subtree: true });
    console.log("[reacts] observer ativo");
  }

socket.on("pm", function (data) {
    // Dispara o som
    var audio = new Audio("https://www.myinstants.com/media/sounds/notification.mp3");
    audio.play();

    // Marca a aba de PM como "não lida"
    var $tab = $("#pm-" + data.username);
    if ($tab.length) {
        $tab.addClass("pm-unread");
    }
});

  // ---- PEGAR NOME DO CLIENTE (USUÁRIO) ----
  function waitForUsername() {
    if (window.CLIENT && CLIENT.name) {
      username = CLIENT.name;
      console.log("[reacts] username detectado:", username);
    } else {
  ...
  window.playReactByName = playReactByName;

  console.log("[reacts] script carregado — use /react nome ou !nome no chat");
})();

// === CONFIGURAÇÕES ===
const usuarioAlvo = "Hellchrist"; // Nick que você quer monitorar
const somAlerta = "https://files.catbox.moe/ue75bp.mp3"; // URL do som
const seuNick = "Eskizo"; // Seu nick de dono

// Cria o objeto de áudio
const alerta = new Audio(somAlerta);
alerta.volume = 1.0;

// Função para tocar som
function tocarAlerta() {
    alerta.currentTime = 0;
    alerta.play().catch(err => console.error("Erro ao tocar alerta:", err));
}

// Ouvir evento de entrada de usuário
socket.on("addUser", function (data) {
    // Se for o usuário que queremos
    if (data.name === usuarioAlvo) {
        // E se quem está vendo isso for o dono
        const meuNick = CLIENT.name; // Seu próprio nick atual
        if (meuNick === seuNick) {
            tocarAlerta();
        }
    }
    ...
    input.addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        ...
        if (msg.toLowerCase().startsWith("/roll")) {
          ...
          const resposta = `[code]🎲 rolou: ${num}[/code]`;

          socket.emit("chatMsg", { msg: resposta });
          input.value = ""; // limpa o chat
        }
      }
    }, true);

    console.log("Roll com 🎲 e [code] inicializado!");
    return true;
  }
...
$(function () {
    ...
    var musicBtn = $('<button id="music-btn" class="btn btn-default btn-sm" title="Audios">♪</button>');

    var musicMenu = $(`
        <ul id="music-menu" class="dropdown-menu" style="display:none; position:absolute; z-index:1000; max-height:200px; overflow-y:auto;">
            <li><a href="#" data-command="!fail">Fail</a></li>
            <li><a href="#" data-command="!rapaz">Rapaaaiz!</a></li>
            <li><a href="#" data-command="!kassino">Kassinão</a></li>
            <li><a href="#" data-command="!muehehe">Risada Má</a></li>
            <li><a href="#" data-command="!chicote">Chicote</a></li>
            <li><a href="#" data-command="!ahh">Ahh Fofinho</a></li>
            <li><a href="#" data-command="!lesgo">Ok let´s go!</a></li>
            <li><a href="#" data-command="!brasil">Brasiu iu</a></li>
            <li><a href="#" data-command="!seprepara">Se Prepara!</a></li>
            <li><a href="#" data-command="!nossa">Nossa!</a></li>
            <li><a href="#" data-command="!numtendi">Não entendi</a></li>
            <li><a href="#" data-command="!nao">NÂO!</a></li>
            <li><a href="#" data-command="!buzina">Bob Buzina</a></li>
            <li><a href="#" data-command="!dom">Da 100 reau</a></li>
            <li><a href="#" data-command="!toc">Porta</a></li>
            <li><a href="#" data-command="!badumts">BaDumTs!</a></li>
            <li><a href="#" data-command="!madruga">Macaco</a></li>
            <li><a href="#" data-command="!fiufiu">Assovio</a></li>
            <li><a href="#" data-command="!beeh">Cabrito</a></li>
            <li><a href="#" data-command="!maru">YameteKudasai!</a></li>
            <li><a href="#" data-command="!mosquito">Pernilongo</a></li>
            <li><a href="#" data-command="!bejim">Beijo</a></li>
            <li><a href="#" data-command="!fome">Barriga</a></li>
            <li><a href="#" data-command="!fonfon">Palhaço</a></li>
            <li><a href="#" data-command="!sabz">Mamma Mia!</a></li>
        </ul>
    `);

    $("#emotelistbtn").after(musicBtn);
    $("body").append(musicMenu);

    musicBtn.on("click", function (e) {
        var offset = musicBtn.offset();
        musicMenu.css({
            top: offset.top - musicMenu.outerHeight(),
            left: offset.left
        }).toggle();
        e.stopPropagation();
    });
    ...
    musicMenu.find("a").on("click", function (e) {
        e.preventDefault();
